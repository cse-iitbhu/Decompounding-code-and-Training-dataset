...
import re
from tensorflow.python.keras.layers import Input, GRU, Dense, Concatenate, TimeDistributed, Bidirectional, LSTM, AdditiveAttention, Attention, SimpleRNN
...
encoder = Bidirectional(SimpleRNN(latent_dim, return_state=True, dropout = dp))
...
decoder_rnn = SimpleRNN(latent_dim*2, return_sequences=True, return_state=True,dropout = dp)
...
dense = Dense(num_tokens, activation='softmax', name='softmax_layer')
...
decoder_state_input_fh = Input(shape=(latent_dim*2,))

encoder_inf_out = Input(shape=(latent_dim*2,))

decoder_states_inputs = [decoder_state_input_fh]

decoder_outputs, state_fh = decoder_rnn(decoder_inputs, initial_state=decoder_states_inputs)
decoder_states = [state_fh]

attn_inf_out = attn_layer([decoder_outputs, encoder_inf_out])
decoder_inf_concat = Concatenate(axis=-1, name='concat')([decoder_outputs, attn_inf_out])

decoder_outputs = dense_time(decoder_inf_concat)
decoder_model = Model([decoder_inputs] + decoder_states_inputs + [encoder_inf_out], [decoder_outputs] + decoder_states)
