...
encoder = Bidirectional(LSTM(latent_dim, return_state=True, dropout = dp))
...
decoder_lstm = LSTM(latent_dim*2, return_sequences=True, return_state=True,dropout = dp)
...
decoder_state_input_fc = Input(shape=(latent_dim*2,))
encoder_inf_out = Input(shape=(latent_dim*2,))

decoder_states_inputs = [decoder_state_input_fh, decoder_state_input_fc]

decoder_outputs, state_fh, state_fc = decoder_lstm(decoder_inputs, initial_state=decoder_states_inputs)
decoder_states = [state_fh, state_fc]
